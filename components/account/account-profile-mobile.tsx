    ...
    <div className="pt-4">
      ...
      >
        <div className="flex items-center space-x-3">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-white font-medium text-sm truncate">{authenticatedUser.name || 'User'}</p>
      ...
      {isExpanded && (
        <div className="mt-3 space-y-2 animate-in slide-in-from-top-2 duration-200">
          ...
          >
            <IoTime size={18} />
            <span className="text-sm font-medium">Recently watched movies</span>
          </Link>
          ...
          >
            <IoLogOut size={18} />
            <span className="text-sm font-medium">{isLoading ? <LoadingSpinerBtn /> : 'Sign out'}</span>
          </button>
        </div>
      )}
    </div>
